...
function PrincipalRoute() {
  const [admin, setAdmina] = useState(false);
  const [waiter, setWaiterw] = useState(false);
  const [chef, setChefc] = useState(false);
  // console.log(admin)
  ...
  return (
    <BrowserRouter>
      <Routes>
        <Route path='/' element={
          <PublicRoutes waiter={waiterLS} admin={adminLS} chef={chefLS}>
            <LoginT />
          </PublicRoutes>
        }></Route>
        ...
        {(adminLS) && (<Route path='/Admin' element={<AppAdmin />}>
